...
The singular values of an [`SVD`](../references/glossary.md#svd) are not just intermediate numerical results — they are physical observables that characterise the quantum state. Keeping them as a first-class object (rather than buried inside a factored matrix) makes entanglement analysis, convergence diagnostics, and [`canonical form`](../references/glossary.md#canonical-form) bookkeeping all natural.

The spectrum hierarchy in Qritical reflects three genuinely different physical situations:

**`SingValSpectrum`** is "just for a matrix": singular values from an [`SVD`](../references/glossary.md#svd) of an array with no further physical context. It carries the discarded weight ``\varepsilon`` and a `normalized` flag (whether ``\sum_i \sigma_i^2 = 1``). This is what lives at each [`bond`](../references/glossary.md#bond) of a `FiniteMPS`.

**`SchmidtSpectrum`** wraps a `SingValSpectrum` with physical meaning: it belongs to a *state bipartition*, records the `Bipartition` (which legs are the cut), and points to the [`orthogonality centre`](../references/glossary.md#orthogonality-centre) (which virtual bond it gauges). [`Entanglement entropy`](../references/glossary.md#entanglement-entropy) and [`Schmidt rank`](../references/glossary.md#schmidt-rank) are only physically meaningful once both the cut and the bond location are specified — demanding a `SchmidtSpectrum` at a function boundary enforces this. A bare `SingValSpectrum` will not typecheck where a `SchmidtSpectrum` is expected.
...
The spectrum verbs — `schmidt_rank`, `spectral_gap`, `schmidt_values`, `entanglement_entropy`, `entanglement_spectrum` — are derived functions on `AbstractSpectrum`, not stored fields. The [`entanglement spectrum`](../references/glossary.md#entanglement-spectrum) is ``-2 \log \sigma_i``; the von Neumann entropy is ``-\sum_i \sigma_i^2 \log \sigma_i^2`` with the ``0 \log 0 = 0`` convention. The [`spectral gap`](../references/glossary.md#spectral-gap) is ``\sigma_1 - \sigma_2``, a measure of how quickly entanglement falls off.

[`Bond`](../references/glossary.md#bond), `OrthoCenter`, `BondCenter`, and `SiteCenter` provide the geometric language for describing where the [`orthogonality centre`](../references/glossary.md#orthogonality-centre) of an [`MPS`](../references/glossary.md#mps) sits:
