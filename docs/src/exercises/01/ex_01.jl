...
# **Why activate `docs/` and not the root project?** #src
# The `docs/` directory has its own `Project.toml` that lists documentation-specific dependencies — `Documenter.jl`, `Literate.jl`, `CairoMakie.jl` for inline plots, `BenchmarkTools.jl`, and so on. The root `Qritical.jl` project only declares the *library's* runtime dependencies (the things a downstream user of Qritical would need), so activating it here would mean `using CairoMakie` blows up with a "not found in current active environment" error. #src
