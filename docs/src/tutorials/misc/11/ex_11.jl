...
# =====================================================================
#
# Week 11 — If a physical *state* is a big tensor of low-rank, then a MPS with small bond dimension can capture it.  Discretised finely enough, f(x) is also a big tensor.
# the quantics trick makes each binary digit of the grid index one "site", so the very
# same tensor-train compression applies. The bond dimension χ that measured
# entanglement for states now measures how compressible a function is across length
# scales. Same machinery, a completely different application.
#
...
function plot_bondprofile(res; title="")
    ...
    ax = Axis(fig[1, 1]; title=title, xlabel=L"bond index $\ell$", ylabel=L"\chi_\ell")
...
println(
    ...
    χa,
    ",   final TCI error ≈ ",
    round(resa.errors[end]; sigdigits=3),
)
println("     bond profile χ_ℓ = ", bondprofile(resa))

save(
    joinpath(@__DIR__, "11_a_overlay.png"),
    plot_overlay(
        fa,
        resa,
        range(-3, 3; length=800);
        title=L"(a)  $e^{-x^2}$ on $[-3,3]$  —  QTT vs exact",
    ),
)
save(
    joinpath(@__DIR__, "11_a_bond.png"),
...
for R in (20, 30, 40)
    ...
    println(
        "     ",
        rpad(R, 5),
        rpad(maximum(bondprofile(res)), 8),
...
save(
    joinpath(@__DIR__, "11_b_full.png"),
    plot_overlay(
        fb,
        resb,
        range(0, log(20); length=3000);
...
win = 30 * 2π * B
save(
    joinpath(@__DIR__, "11_b_zoom.png"),
    plot_overlay(
        fb,
        resb,
        range(0, win; length=2000);
        every=8,
...
save(
    joinpath(@__DIR__, "11_b_bond.png"),
    plot_bondprofile(
        resb;
...
save(
    joinpath(@__DIR__, "11_c_overlay.png"),
    plot_overlay(
        fc,
        resc,
        range(-3, 3; length=600);
        every=2,
        title=L"(c)  noisy Gaussian ($\eta=0.05$)  —  QTT reproduces every jitter",
    ),
)
save(
    joinpath(@__DIR__, "11_c_bond.png"),
    plot_bondprofile(
        resc; title=L"(c)  bond profile saturates to $2^{\lfloor R/2\rfloor}=%$(2^(Rc÷2))$"
    ),
)
...
for (τ, χ, e) in zip(tols, χ_of_tol, err_of_tol)
    println(
        "     τ = ", rpad(τ, 9), "  χ = ", rpad(χ, 4), "  err ≈ ", round(e; sigdigits=3)
    )
end
let fig = Figure(; size=(760, 340))
    ax = Axis(
        fig[1, 1];
        title=L"(a)  cost vs accuracy at $R=20$",
        xlabel=L"requested tolerance $\tau$",
        ylabel=L"\chi_{\max}",
        xscale=log10,
        xreversed=true,
    )
    ...
    save(joinpath(@__DIR__, "11_explore_tol.png"), fig)
end
...
let fig = Figure(; size=(760, 340))
    ax = Axis(
        fig[1, 1];
        title=L"(b)  error vs bond cap $D$ at $R=30$",
        xlabel=L"maximum bond dimension $D$",
        ylabel=L"final TCI error",
        yscale=log10,
    )
    lines!(ax, Ds, err_of_D; color=:darkorange, linewidth=2)
    scatter!(ax, Ds, err_of_D; color=:darkorange, markersize=8)
    save(joinpath(@__DIR__, "11_explore_D.png"), fig)
end
