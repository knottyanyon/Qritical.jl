...
### Physics behavior tests (write first)
- [ ] Full SVD: `‖A - U diag(S) Vd‖ < tol` (exact reconstruction when no truncation)
- [ ] Singular values are non-negative and sorted descending
- [ ] `U†U ≈ I` (left isometry)
- [ ] `Vd Vd† ≈ I` (right isometry)
- [ ] `KeepFirst(r)`: output has exactly `r` singular values
- [ ] `KeepAbove(atol)`: every retained σ satisfies `σ > atol`; every discarded σ satisfies `σ ≤ atol`
- [ ] `KeepRelative(rtol)`: every retained σ satisfies `σ / σ_max > rtol`
- [ ] Truncation error `ε = ‖discarded singular values‖₂` is returned and numerically correct
- [ ] `KeepFirst(r)` with `r ≥ rank(A)`: no truncation, `ε = 0`

### Types
- [ ] `Bisection` — left/right partition of an index tuple
- [ ] `AbstractTruncation`, `KeepFirst`, `KeepAbove`, `KeepRelative`
- [ ] `BondIndex` — virtual bond produced by SVD, carries `ndim` = number of retained singular values

### Algorithms
- [ ] `tensor_svd(A::IndexedTensor, b::Bisection, trunc::AbstractTruncation)` — native backend only; returns `(U, S, Vd, ε)`
- [ ] Re-indexing: `U` carries the original left index tuple + new `BondIndex`; `Vd` carries new `BondIndex` + original right index tuple

### Edge case tests
- [ ] Rank-deficient input: `KeepFirst(r)` with `r > rank(A)` keeps only non-zero singular values
- [ ] `1 × N` and `N × 1` bisections (all indices on one side)
- [ ] Complex-valued tensor: singular values are real
- [ ] `KeepAbove(0.0)`: identical to keeping all singular values
...
### Physics behavior tests (write first)
- [ ] `hilbert_space(Spin{1//2}())` has dimension 2
- [ ] `hilbert_space(Fermionic())` has dimension 2
- [ ] `hilbert_space(HardCoreBoson())` has dimension 2
- [ ] Fresh `FiniteMPS` from random tensors has `form == ArbitraryForm()`
- [ ] After full left sweep: `form == CanonicalForm(L, L+1)` and `‖A[i]† A[i] - I‖ < tol` for all `i`
- [ ] After full right sweep: `form == CanonicalForm(0, 1)` and `‖B[i] B[i]† - I‖ < tol` for all `i`
- [ ] Left-canonical and right-canonical forms represent the same state: `|⟨ψ_L | ψ_R⟩|² ≈ 1`
- [ ] `⟨ψ|ψ⟩ = 1` for a normalized MPS
- [ ] Moving orthogonality center one step: `llim`/`rlim` changes by ±1, state is preserved
- [ ] `bond_svs[1] == bond_svs[L+1] == [1.0]` (trivial boundary)
- [ ] Entanglement entropy of a product state (χ = 1 everywhere) is 0 at every bond
- [ ] Entropy formula: `S = -∑ λᵢ² log(λᵢ²)` applied to `bond_svs[i]` matches contraction-based result

### Types
- [ ] `AbstractDoF`, `Spin{S}`, `Fermionic`, `HardCoreBoson`
- [ ] `StateSite{D <: AbstractDoF}`
- [ ] `AbstractMPSForm`, `CanonicalForm(llim, rlim)`, `ArbitraryForm()`
- [ ] `FiniteMPS{D <: AbstractDoF, T <: Number}`

### Algorithms
- [ ] `hilbert_space(::AbstractDoF)` — returns local Hilbert space dimension (native: plain `Int`; TensorKit: deferred)
- [ ] `left_canonical_sweep!(mps)` — updates `form` to `CanonicalForm(L, L+1)`
- [ ] `right_canonical_sweep!(mps)` — updates `form` to `CanonicalForm(0, 1)`
- [ ] `move_center!(mps, i)` — shifts orthogonality center to site `i`
- [ ] `overlap(ψ, φ)` — `⟨ψ|φ⟩` via boundary-to-boundary contraction
- [ ] `entanglement_entropy(mps, bond)` — from `bond_svs[bond]`
- [ ] `+(ψ, φ)` — MPS addition via block-diagonal tensor concatenation

### Edge case tests
- [ ] `L = 1`: canonicalization is trivially the identity; `⟨ψ|ψ⟩` = squared Frobenius norm of single tensor
- [ ] Complex MPS: `⟨ψ|ψ⟩` is real and positive
- [ ] `CanonicalForm(1, 2)` and `CanonicalForm(L-1, L)` — center at left/right boundary sites
- [ ] `KeepFirst(1)` during sweep on an entangled state: state becomes product state, entropy = 0
...
- **`:tensorkit` backend** — block-sparse contractions with symmetry-sector quantum numbers; requires sector information on `BondIndex`
