...
@testitem "§9.1 disorder_realization" begin
    @testitem "disorder_realization returns correct length" begin
        h = disorder_realization(6, Uniform(-1.0, 1.0), MersenneTwister(42))
        @test length(h) == 6
    end

    @testitem "disorder_realization is reproducible with same seed" begin
    ...
    @testitem "Uniform distribution samples within bounds" begin
        h = disorder_realization(100, Uniform(-1.0, 1.0), MersenneTwister(7))
        @test all(-1.0 .≤ h .≤ 1.0)
    end

    @testitem "XXZ accepts disorder field array" begin
        g = Chain(4)
    ...
    @testitem "disorder Hamiltonian is Hermitian" begin
        g = Chain(4)
...
@testitem "§9.1 imaginary-time GS convergence" begin
    @testitem "imaginary-time TEBD converges to ED GS energy" begin
        g = Chain(4)
    ...
    @testitem "disordered model: imaginary-time lowers energy below random initial" begin
        g = Chain(4)
    ...
    @testitem "MBL signature: disorder suppresses entanglement growth" begin
        ...
        r_clean = solve(
            H_clean,
            Evolution(ψ₀_clean),
            TEBD(SuzukiTrotter(2), MaxBondDimTrunc(32)),
            p_clean,
        )
        r_dis = solve(
            H_dis, Evolution(ψ₀_dis), TEBD(SuzukiTrotter(2), MaxBondDimTrunc(32)), p_dis
        )

        # Entropy at center bond
        center = div(L, 2)
        function center_entropy(ψ)
            # The LeftCanonical pass is load-bearing, not tidying. `BondCanonical(k)` writes
            # bond_svs[k+1] from its RIGHT sweep, whose SVD at site k+1 only yields Schmidt
            # values if everything to its left is already left-canonical — and BondCanonical
            # itself only makes sites 1..k-1 so, leaving the centre site k to contaminate the
            # spectrum. Sweeping left-canonical first supplies the missing condition. Feeding
            # it an arbitrarily-gauged state used to give roughly the right answer by luck;
            # it stopped once TEBD started handing back a state whose right half is already
            # right-canonical, which makes that right sweep a no-op returning a flat spectrum
            # of ones (entropy = log2 χ, saturated, identical for every state).
            ψc = canonicalize(
                canonicalize(ψ, LeftCanonical(NoTrunc())), BondCanonical(center, NoTrunc())
            )
    ...
    @testitem "⟨S₁ᶻSᵢᶻ⟩ of TEBD GS matches ED cross-check" begin
        g = Chain(4)
...
@testitem "§9.1 J-sweep driver" begin
    @testitem "parameter_sweep returns a result for each J value" begin
        J_vals = [0.5, 1.0, 2.0]
        g = Chain(4)
    ...
    @testitem "GS energy is monotone in J for Heisenberg chain" begin
    ...
    @testitem "disorder-averaged energy from parameter_sweep is self-consistent" begin
        ...
        seeds = 1:n
        avg_E = mean(
            parameter_sweep(collect(seeds)) do seed
