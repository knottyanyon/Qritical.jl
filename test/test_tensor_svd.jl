...
@testset "tensor_svd: physics behavior" begin
    ...
    (; U, S, Vd, ε) = tensor_svd(A, bp, KeepFirst(r))
    ...
    @testset "full SVD reconstructs original matrix" begin
        U_mat  = reshape(U.data,  :, length(S))
        Vd_mat = reshape(Vd.data, length(S), :)
        @test norm(M.data - U_mat * Diagonal(S) * Vd_mat) / norm(M.data) < 1e-12
    end
...
# ── tensor_svd: BondIndex re-indexing ─────────────────────────────────────────

@testset "tensor_svd: BondIndex re-indexing" begin
    A  = _site_tensor()
    bp = _site_bipartition()
    r  = 3
    (; U, S, Vd) = tensor_svd(A, bp, KeepFirst(r))

    @testset "U has left-partition indices followed by BondIndex" begin
        @test ndims(U)         == 3             # vL, σ, bond
        @test U.indices[1]     == upper(:vL, 2)
        @test U.indices[2]     == lower(:σ, 3)
        @test U.indices[end]   isa BondIndex
        @test ndim(U.indices[end]) == r
    end

    @testset "Vd has BondIndex followed by right-partition indices" begin
        @test ndims(Vd)          == 2           # bond, vR
        @test Vd.indices[1]      isa BondIndex
        @test ndim(Vd.indices[1]) == r
        @test Vd.indices[2]      == upper(:vR, 4)
    end

    @testset "U and Vd share the same BondIndex" begin
        @test U.indices[end] == Vd.indices[1]
    end
...
@testset "tensor_svd: truncation" begin
    ...
    @testset "KeepRelative(rtol): all retained σ satisfy σ/σ_max > rtol" begin
        rtol = 0.3
        (; S) = tensor_svd(A, bp, KeepRelative(rtol))
        @test all(s -> s / all_svs[1] > rtol, S)
    end

    @testset "KeepRelative(rtol): ε equals 2-norm of discarded singular values" begin
        rtol = 0.3
        (; S, ε) = tensor_svd(A, bp, KeepRelative(rtol))
...
@testset "tensor_svd: edge cases" begin
    ...
    @testset "KeepFirst(r ≥ rank): no truncation, ε = 0" begin
        ...
        bp = _site_bipartition()
        M  = group_legs(A, bp)
        (; S, ε) = tensor_svd(A, bp, KeepFirst(100))   # 100 >> actual rank
        @test length(S) == rank(M.data)
        @test ε == 0
    end
    ...
    @testset "rank-deficient input: only non-zero singular values kept" begin
        ...
        A    = IndexedTensor(data, (upper(:row, 4), upper(:col, 3)))
        bp   = Bipartition(Partition(upper(:row, 4)), Partition(upper(:col, 3)))
        (; S, ε) = tensor_svd(A, bp, KeepFirst(3))   # request 3, rank is 1
        @test length(S) == 1
        @test ε ≈ 0 atol=1e-12
    end
    ...
    @testset "1×N bipartition: single index on left side" begin
        ...
        M   = group_legs(A, bp)
        @test size(M) == (2, 12)
        (; U, S, Vd) = tensor_svd(A, bp, KeepFirst(2))
        U_mat  = reshape(U.data,  :, length(S))
        Vd_mat = reshape(Vd.data, length(S), :)
        @test norm(M.data - U_mat * Diagonal(S) * Vd_mat) / norm(M.data) < 1e-12
    end
    ...
    @testset "N×1 bipartition: single index on right side" begin
        A   = _site_tensor()
        bp  = Bipartition(Partition(upper(:vL, 2), lower(:σ, 3)), Partition(upper(:vR, 4)))   # 6 × 4
        M   = group_legs(A, bp)
        @test size(M) == (6, 4)
        (; U, S, Vd) = tensor_svd(A, bp, KeepFirst(4))
        U_mat  = reshape(U.data,  :, length(S))
        Vd_mat = reshape(Vd.data, length(S), :)
        @test norm(M.data - U_mat * Diagonal(S) * Vd_mat) / norm(M.data) < 1e-12
    end
    ...
    @testset "KeepAbove(0.0): retains all singular values" begin
        ...
        M        = group_legs(A, bp)
        all_svs  = svdvals(M.data)
        (; S, ε) = tensor_svd(A, bp, KeepAbove(0.0))
        @test length(S) == length(all_svs)
        @test ε == 0
    end
