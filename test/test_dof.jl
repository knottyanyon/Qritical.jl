@testset "§6.1 DoF layer — types, operators, statistics" begin
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(SpinHalf()) — NamedTuple keys" begin
        ops = operators(Spin{1//2}())
        @test haskey(ops, :Sx)
        @test haskey(ops, :Sy)
        @test haskey(ops, :Sz)
        @test haskey(ops, :Sp)
    ...
    @testset "operators(SpinHalf()) — matrix sizes are 2×2" begin
        ops = operators(Spin{1//2}())
    ...
    @testset "operators(SpinHalf()) — Sᶻ eigenvalues are ±½" begin
        ops = operators(Spin{1//2}())
        ev = sort(real.(eigvals(ops.Sz)))
        @test ev ≈ [-0.5, 0.5] atol=1e-12
    end

    @testset "operators(SpinHalf()) — SU(2) commutation relations" begin
        ops = operators(Spin{1//2}())
    ...
    @testset "operators(SpinHalf()) — ladder operator adjoint (S⁺)† = S⁻" begin
        ops = operators(Spin{1//2}())
        @test ops.Sp' ≈ ops.Sm  atol=1e-12
        @test ops.Sm' ≈ ops.Sp  atol=1e-12
    end

    @testset "operators(SpinHalf()) — ladder operators raise/lower Sᶻ" begin
        ops = operators(Spin{1//2}())
    ...
    @testset "operators(SpinHalf()) — Casimir S² = s(s+1)I = 3/4 I" begin
        ops = operators(Spin{1//2}())
        S2 = ops.Sx^2 + ops.Sy^2 + ops.Sz^2
        @test S2 ≈ (3/4) * ops.I  atol=1e-12
    end

    @testset "operators(SpinHalf()) — Sx, Sz are Hermitian; Sy is Hermitian" begin
        ops = operators(Spin{1//2}())
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(Spin{1}()) — 3×3 matrices with Casimir s(s+1)=2" begin
        ops = operators(Spin{1}())
        for name in (:Sx, :Sy, :Sz, :Sp, :Sm, :I)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(SpinlessFermion()) — CAR: {c, c†} = I, {c,c} = 0" begin
    ...
    @testset "operators(SpinlessFermion()) — n eigenvalues are 0 and 1" begin
        ops = operators(SpinlessFermion())
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(HardCoreBoson()) — b² = 0, {b,b†} = I" begin
        ops = operators(HardCoreBoson())
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(Electron()) — 4×4 sizes" begin
    ...
    @testset "operators(Electron()) — CAR within each spin species" begin
        ops = operators(Electron())
        # {c↑, c†↑} = I, {c↓, c†↓} = I
    ...
    @testset "operators(Electron()) — nup, ndn, n eigenvalues" begin
        ops = operators(Electron())
    ...
    @testset "operators(Electron()) — standard intra-site sign: c↓|↑↓⟩ = −|↑⟩" begin
        ops = operators(Electron())
        # basis: {|0⟩=e1, |↑⟩=e2, |↓⟩=e3, |↑↓⟩=e4}
    ...
    # ─────────────────────────────────────────────────────────────────────────
    @testset "operators(Majorana()) — {γₐ, γᵦ} = 2δ" begin
        ops = operators(Majorana())
        # {γ1, γ1} = 2I
    ...
    @testset "operators(Majorana()) — both are Hermitian (γ†=γ)" begin
        ops = operators(Majorana())
