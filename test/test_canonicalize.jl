...
@testset "CanonicalizeConfig and canonicalize" begin
    ...
    @testset "truncating canonicalize accumulates ε from per-bond errors" begin
        ...
        # Sum truncation errors from bond 2 to bond L (skip trivial boundary bonds 1 and L+1).
        # Generator expression: `sum(f(i) for i in range)` = Python's `sum(f(i) for i in range)`.
        @test result.ε ≥ 0.0
        # Non-negativity: truncation error is always non-negative (you discard weight, not gain it).
        @test result.ε ≈ per_bond_ε atol=1e-14
        # The total error stored in mps.ε must exactly equal the sum of individual bond errors.
        # Physics: error accounting is additive — total approximation error = sum of bond approximations.
    end
end
